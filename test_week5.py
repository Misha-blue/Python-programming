...
from client_good import Client, ClientError
...
class ServerSocket:
    ...
    def send(self, data):
        if data in self.rsp_map:
            ...
            raise ServerSocketException("запрос не соответствует протоколу: {}".format(data))
...
class TestClient(unittest.TestCase):
        ...
        for metric, value, timestamp in metrics_for_put:
            try:
                ...
                self.fail("Ошибка вызова client.put("
                          "'{}', {}, timestamp={})\n{}".format(metric, value, timestamp, message))
    ...
    def test_client_get_client_error(self):
        try:
            ...
            message = exp.args[0]
            self.fail("Некорректно обработано сообщение сервера об ошибке: {}".format(message))
