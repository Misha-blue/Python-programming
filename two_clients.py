...
"""
import sys
from client_good import Client, ClientSocketError, ClientProtocolError
...
def run(host, port):
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('*') вернул неверный результат. Ожидается: {}. Получено: {}".format(expected_metrics,
                                                                                                  metrics))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.put(...) {}: {}".format(err.__class__, err))
    ...
    try:
        ...
        if metrics != expected_metrics:
            print("client.get('k2') вернул неверный результат. Ожидается: {}. Получено: {}".format(expected_metrics,
                                                                                                   metrics))
            sys.exit(1)
    except Exception as err:
        print("Ошибка вызова client.put(...) {}: {}".format(err.__class__, err))
    ...
    try:
        ...
        if result != {}:
            print("Ошибка вызова метода get с ключом, который еще не был добавлен. Ожидается: пустой словарь. "
                  "Получено: {}".format(result))
            sys.exit(1)
    except Exception as err:
